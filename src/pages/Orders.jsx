      ...
      for (let i = 0; i < styleNumbers.length; i++) {
        ...
        try {
          ...
          records.forEach((record) => {
            ...
            // Since NocoDB doesn't have fabric_number, we'll distribute based on available fabrics
            let fabricKey = 'Unknown';

            // If we have fabric details, try to match by some logic
            if (fabricDetailsForStyle.length === 1) {
              // If only one fabric, assign to that
              fabricKey = String(fabricDetailsForStyle[0].fabricNumber);
            } else if (fabricDetailsForStyle.length > 1) {
              // Multiple fabrics - we need to determine which one
              // Check if record has any fabric related field
              ...
              for (const field of possibleFabricFields) {
                if (record[field] !== undefined && record[field] !== null && record[field] !== '') {
                  ...
                  // Check if this value matches any fabric in our list
              ...
              // If still not found, try to match by channel or size patterns
              if (!foundFabric) {
                // Simple round-robin distribution based on order index
                // This is a fallback, better than assigning all to Unknown
            ...
            // Ensure fabric key exists in groups
            if (!fabricGroups[fabricKey]) {
              // Create a new group for this fabric
          ...
          // Update channel-wise counts (excluding unwanted channels)
          ...
          // Calculate if fabric is sufficient
        ...
        <div className="bg-white rounded-lg p-6 mb-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors disabled:opacity-50 flex items-center gap-2 text-sm"
            >
        ...
        <div className="bg-white rounded-lg p-6 mb-6">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={calculating}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center gap-1 text-sm"
                >
                  <Search size={18} />
                  Add
                </button>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <label className="flex-1 cursor-pointer">
                  <div className="px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-500 transition-colors text-center text-gray-500 hover:text-blue-600 text-sm">
          ...
          <div className="flex flex-wrap gap-4 mt-6">
            <button
              ...
              className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
            >
              {calculating ? (
                <>
            ...
            {results.length > 0 && (
              <button
                onClick={exportCSV}
                disabled={calculating}
                className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2 text-sm"
              >
                <Download size={18} />
                Export CSV
              </button>
        ...
        {results.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
            <div className="bg-white rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-gray-900">{totals.totalStyles}</p>
            </div>
            <div className="bg-white rounded-lg p-4">
              <p className="text-xs text-gray-500">Total Orders</p>
              <p className="text-xl font-bold text-blue-600">{totals.totalOrders}</p>
            </div>
            <div className="bg-white rounded-lg p-4">
            ...
            <div className="bg-white rounded-lg p-4">
              ...
              <p className="text-xl font-bold text-purple-600">{totals.stylesWithScans}</p>
            </div>
            <div className="bg-white rounded-lg p-4">
              ...
              <div className="p-6 border-b sticky top-0 bg-white z-10">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">{selectedStyle.dateRange}</p>
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs text-gray-500">Total Orders</p>
                    <p className="text-lg font-bold">{selectedStyle.orderCount}</p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <p
                      className={`text-lg font-bold ${selectedStyle.fabricSufficient ? 'text-green-600' : 'text-red-600'}`}
                    >
                ...
                {/* Fabric-wise Breakdown - Per Fabric Number */}
                <div className="mb-6">
                      ...
                      <div key={idx} className="border rounded-lg overflow-hidden">
                        ...
                        {expandedFabric === fabric.fabricNumber && (
                          <div className="p-4 border-t">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* Size Distribution for this fabric */}
                              ...
                              {/* Channel Distribution for this fabric */}
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(selectedStyle.sizeDistribution).map(([size, count]) => (
                        <div key={size} className="bg-purple-50 px-3 py-2 rounded-lg">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {selectedStyle.scanDetails.slice(0, 20).map((scan, idx) => (
                            <tr key={idx}>
                              <td className="px-3 py-2">{scan.orderId}</td>
                              <td className="px-3 py-2">{scan.size}</td>
                              <td className="px-3 py-2">{scan.channel}</td>
                              <td className="px-3 py-2">{scan.fabricNumber}</td>
                              <td className="px-3 py-2">{scan.fabricName}</td>
                              <td className="px-3 py-2">{scan.fabricUsed.toFixed(2)}m</td>
